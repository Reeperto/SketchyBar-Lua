...
static inline void notification_destroy(struct notification *notification) {
    if (notification->name)
        free(notification->name);
    if (notification->info)
        free(notification->info);
    free(notification);
}
...
static inline char *format_bool(bool b) { return b ? "on" : "off"; }
...
static inline struct key_value_pair get_key_value_pair(char *token,
                                                       char split) {
...
static inline void pack_key_value_pair(char *cursor,
                                       struct key_value_pair *key_value_pair) {
...
static inline bool is_root(void) { return getuid() == 0 || geteuid() == 0; }
...
static inline char **token_split(struct token token, char split,
                                 uint32_t *count) {
...
static inline bool evaluate_boolean_state(struct token state,
                                          bool previous_state) {
...
static inline void clip_rect(CGContextRef context, CGRect region, float clip,
...
static inline CFArrayRef cfarray_of_cfnumbers(void *values, size_t size,
    ...
    CFArrayRef result =
        CFArrayCreate(NULL, (const void **)temp, count, &kCFTypeArrayCallBacks);
...
static inline char *cfstring_copy(CFStringRef string) {
    ...
    if (!CFStringGetCString(string, result, num_bytes + 1,
