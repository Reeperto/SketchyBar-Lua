...
static inline void env_vars_unset(struct env_vars *env_vars, char *key) {
    ...
    if (env_vars->count == 1) {
        ...
        int count = 0;
        for (int i = 0; i < env_vars->count; i++) {
            if (env_vars->vars[i] == key_value_pair)
                continue;
            tmp[count++] = env_vars->vars[i];
        }
        env_vars->count--;
        env_vars->vars = realloc(
            env_vars->vars, sizeof(struct key_value_pair *) * env_vars->count);

        memcpy(env_vars->vars, tmp,
...
static inline void env_vars_set(struct env_vars *env_vars, char *key,
                                char *value) {
...
static inline char *env_vars_get_value_for_key(struct env_vars *env_vars,
                                               char *key) {
    for (int i = 0; i < env_vars->count; i++) {
        if (strcmp(env_vars->vars[i]->key, key) == 0)
...
static inline char *
env_vars_copy_serialized_representation(struct env_vars *env_vars,
                                        uint32_t *len) {
    uint32_t length = 0;
    for (int i = 0; i < env_vars->count; i++) {
        length +=
            env_vars->vars[i]->key ? strlen(env_vars->vars[i]->key) + 1 : 1;

        length +=
            env_vars->vars[i]->value ? strlen(env_vars->vars[i]->value) + 1 : 1;
    }
    ...
    for (int i = 0; i < env_vars->count; i++) {
        if (env_vars->vars[i]->key) {
            ...
            memcpy(seri + caret, env_vars->vars[i]->key, len);
        ...
        if (env_vars->vars[i]->value) {
            ...
            memcpy(seri + caret, env_vars->vars[i]->value, len);
