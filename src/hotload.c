...
void hotload_set_state(int state) { g_hotload = state; }

int hotload_get_state() { return g_hotload; }
...
void exec_config_file() {
    if (!*g_config_file && !get_config_file("sketchybar.lua", g_config_file,
    ...
    if (luaL_dofile(g_bar_manager.L, g_config_file)) {
        printf("Failed to execute config: \n\n%s\n",
               lua_tostring(g_bar_manager.L, -1));
        return;
    }
}

static void handler(ConstFSEventStreamRef stream, void *context, size_t count,
...
int begin_receiving_config_change_events() {
    ...
    CFStringRef file_ref = CFStringCreateWithCString(kCFAllocatorDefault, file,
                                                     kCFStringEncodingUTF8);

    CFArrayRef paths = CFArrayCreate(NULL, (const void **)&file_ref, 1,
                                     &kCFTypeArrayCallBacks);

    FSEventStreamRef stream = FSEventStreamCreate(
        kCFAllocatorDefault, handler, NULL, paths,
