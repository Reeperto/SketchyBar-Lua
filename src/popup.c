...
static void popup_order_windows(struct popup *popup) {
    int level = popup->topmost ? (kCGScreenSaverWindowLevel + 1)
...
static void popup_calculate_popup_anchor_for_bar_item(struct popup *popup,
                                                      struct bar_item *bar_item,
                                                      struct bar *bar) {
    if (popup->adid != g_bar_manager.active_adid)
        return;
    struct window *window = bar_item_get_window(bar_item, popup->adid);
...
void popup_calculate_bounds(struct popup *popup, struct bar *bar) {
    ...
    for (int j = 0; j < popup->num_items; j++) {
        ...
        if (popup->adid > 0) {
            CGRect frame = {{popup->anchor.x + item_x, popup->anchor.y + y},
                            {bar_item_display_length, item_height}};
        ...
        if (item_width > width && !popup->horizontal)
            width = item_width;
        if (popup->horizontal)
            x += item_width;
        else
    ...
    for (int j = 0; j < popup->num_items; j++) {
        ...
        window_set_frame(
            bar_item_get_window(bar_item->group->members[0], popup->adid),
...
static void popup_create_window(struct popup *popup) {
    ...
    if (popup == &g_bar_manager.default_item.popup)
        return;

    window_create(&popup->window,
                  (CGRect){{popup->anchor.x, popup->anchor.y},
                           {popup->background.bounds.size.width,
                            popup->background.bounds.size.height}});
...
void popup_add_item(struct popup *popup, struct bar_item *bar_item) {
    popup->num_items++;
    popup->items =
        realloc(popup->items, sizeof(struct bar_item *) * popup->num_items);
    popup->items[popup->num_items - 1] = bar_item;
    bar_item->parent = popup->host;
    popup->needs_ordering = true;
}

static bool popup_contains_item(struct popup *popup,
                                struct bar_item *bar_item) {
    for (int i = 0; i < popup->num_items; i++) {
        if (popup->items[i] == bar_item)
            return true;
    }
    return false;
}
...
void popup_draw(struct popup *popup) {
    if (!popup->drawing || popup->adid < 1 || popup->num_items == 0)
        return;
...
void popup_serialize(struct popup *popup, char *indent, FILE *rsp) {
    ...
    fprintf(rsp,
            ...
            "%s\"align\": \"%s\",\n"
            "%s\"background\": {\n",
            indent, format_bool(popup->drawing), indent,
            format_bool(popup->horizontal), indent,
            popup->overrides_cell_size ? popup->cell_size : -1, indent,
            popup->blur_radius, indent, popup->y_offset, indent, align, indent);
...
static bool popup_set_cell_size(struct popup *popup, int size) {
    if (popup->cell_size == size && popup->overrides_cell_size)
        return false;
...
bool popup_parse_sub_domain(struct popup *popup, FILE *rsp,
                            struct token property, char *message) {
    bool needs_refresh = false;
    if (token_equals(property, PROPERTY_YOFFSET)) {
        ANIMATE(popup_set_yoffset, popup, popup->y_offset,
                token_to_int(get_token(&message)));
    } else if (token_equals(property, PROPERTY_DRAWING)) {
        return popup_set_drawing(
            popup, evaluate_boolean_state(get_token(&message), popup->drawing));
    } else if (token_equals(property, PROPERTY_HORIZONTAL)) {
        popup->horizontal =
            evaluate_boolean_state(get_token(&message), popup->horizontal);
        return true;
        ...
        ANIMATE(popup_set_cell_size, popup, popup->cell_size,
                token_to_int(get_token(&message)));
    } else if (token_equals(property, PROPERTY_BLUR_RADIUS)) {
        ANIMATE(popup_set_blur_radius, popup, popup->blur_radius,
                token_to_int(get_token(&message)));
        return false;
    } else if (token_equals(property, PROPERTY_TOPMOST)) {
        return popup_set_topmost(
            popup, evaluate_boolean_state(get_token(&message), popup->topmost));
    } else {
        ...
        if (key_value_pair.key && key_value_pair.value) {
            ...
            if (token_equals(subdom, SUB_DOMAIN_BACKGROUND))
                return background_parse_sub_domain(&popup->background, rsp,
                                                   entry, message);
            else {
