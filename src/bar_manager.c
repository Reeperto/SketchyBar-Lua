...
void bar_manager_init(struct bar_manager *bar_manager) {
    ...
    bar_manager->L = create_lua_state();
    ...
    bar_manager->clock = CFRunLoopTimerCreate(
        NULL, CFAbsoluteTimeGetCurrent() + shell_refresh_frequency,
        shell_refresh_frequency, 0, 0, clock_handler, NULL);

    CFRunLoopAddTimer(CFRunLoopGetMain(), bar_manager->clock,
...
int bar_manager_get_item_index_by_address(struct bar_manager *bar_manager,
...
void bar_manager_move_item(struct bar_manager *bar_manager,
                           ...
                           bool before) {
    if (bar_manager->bar_item_count <= 0)
        return;
    struct bar_item *tmp[bar_manager->bar_item_count];
    int count = 0;
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        if (bar_manager->bar_items[i] == item)
            continue;
    ...
    bar_manager->bar_items =
        realloc(bar_manager->bar_items,
                sizeof(struct bar_item *) * bar_manager->bar_item_count);

    memcpy(bar_manager->bar_items, tmp,
           sizeof(struct bar_item *) * bar_manager->bar_item_count);
...
void bar_manager_remove_item(struct bar_manager *bar_manager,
    ...
    bar_item_destroy(bar_item, true, bar_manager->L);
}

bool bar_manager_set_margin(struct bar_manager *bar_manager, int margin) {
    if (bar_manager->margin == margin)
        return false;
...
bool bar_manager_set_y_offset(struct bar_manager *bar_manager, int y_offset) {
    if (bar_manager->background.y_offset == y_offset)
        return false;
...
bool bar_manager_set_bar_height(struct bar_manager *bar_manager, int height) {
    bar_manager->bar_needs_resize |=
        background_set_height(&bar_manager->background, height);
    return bar_manager->bar_needs_resize;
}

bool bar_manager_set_background_blur(struct bar_manager *bar_manager,
                                     uint32_t radius) {
    if (bar_manager->blur_radius == radius)
        return false;
...
bool bar_manager_set_displays(struct bar_manager *bar_manager,
                              uint32_t displays) {
    if (bar_manager->displays == displays)
        return false;
...
bool bar_manager_set_notch_width(struct bar_manager *bar_manager,
                                 uint32_t width) {
    if (bar_manager->notch_width == width)
        return false;
...
bool bar_manager_set_notch_offset(struct bar_manager *bar_manager,
                                  uint32_t offset) {
    if (bar_manager->notch_offset == offset)
        return false;
...
bool bar_manager_set_font_smoothing(struct bar_manager *bar_manager,
                                    bool smoothing) {
    if (bar_manager->font_smoothing == smoothing)
        return false;
    bar_manager->font_smoothing = smoothing;
    for (int i = 0; i < bar_manager->bar_count; i++)
        context_set_font_smoothing(bar_manager->bars[i]->window.context,
                                   smoothing);
    return true;
}

bool bar_manager_set_hidden(struct bar_manager *bar_manager, uint32_t adid,
                            bool hidden) {
    bar_manager->any_bar_hidden = false;
    if (adid > 0) {
...
uint32_t bar_manager_length_for_bar_side(struct bar_manager *bar_manager,
    ...
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        ...
        if (bar_item->position == side &&
            ...
            total_length +=
                item_length + (bar_item->has_const_width
                                   ? 0
...
bool bar_manager_bar_needs_redraw(struct bar_manager *bar_manager,
                                  struct bar *bar) {
    if (bar_manager->bar_needs_update)
        return true;
    ...
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        ...
        bool regular_update = bar_item->needs_update && draws_item;

        if (regular_update)
            return true;

        bool disabled_item_drawn_on_bar =
            !bar_item->drawing && (bar_item->associated_bar != 0);

        if (disabled_item_drawn_on_bar)
            return true;
        ...
        bool not_drawn_on_associated_display =
            (draws_item && (bar_item->associated_display > 0) &&
             (bar_item->associated_display & bar_mask) &&
             !(((bar_item->associated_bar << 1) & bar_mask))) ||
            (draws_item && bar_item->associated_to_active_display &&
             bar_manager->active_adid == bar->adid &&
             !((bar_item->associated_bar << 1) & bar_mask));

        if (not_drawn_on_associated_display)
            return true;
        ...
        bool drawn_on_non_associated_display =
            (!bar_item->associated_to_active_display &&
             ...
             !(bar_item->associated_display & bar_mask) &&
             (((bar_item->associated_bar << 1) & bar_mask))) ||
            (bar_item->drawing && bar_item->associated_to_active_display &&
             (((bar_item->associated_bar << 1) & bar_mask)) &&
             (bar->adid != bar_manager->active_adid));

        if (drawn_on_non_associated_display)
            return true;

        if (bar_item->type == BAR_COMPONENT_SPACE)
            continue;

        bool drawn_on_non_associated_space =
            bar_item->associated_space > 0 &&
            !(bar_item->associated_space & (1 << bar->sid)) &&
            ((bar_item->associated_bar << 1) & bar_mask);

        if (drawn_on_non_associated_space)
            return true;

        bool not_drawn_on_associated_space =
            draws_item && bar_item->associated_space > 0 &&
            (bar_item->associated_space & (1 << bar->sid)) &&
            !((bar_item->associated_bar << 1) & bar_mask);

        if (not_drawn_on_associated_space)
            return true;
    }
    return false;
}
...
void bar_manager_update_alias_components(struct bar_manager *bar_manager,
                                         bool forced) {
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        ...
        if (!bar_item->overrides_association) {
            ...
            if (space_did) {
                bar_item->associated_display =
                    1 << (display_arrangement(space_did));
            } else {
            ...
            if ((1 << did) & bar_item->associated_display) {
                ...
                if ((!bar_item->selected || forced) &&
                    ...
                    env_vars_set(&bar_item->signal_args.env_vars,
                                 string_copy("SELECTED"), string_copy("true"));
                } else if ((bar_item->selected || forced) &&
                           !(bar_item->associated_space & (1 << sid))) {
                    bar_item->selected = false;
                    bar_item->updates = true;
                    env_vars_set(&bar_item->signal_args.env_vars,
                                 string_copy("SELECTED"), string_copy("false"));
                } else {
...
void bar_manager_animator_refresh(struct bar_manager *bar_manager) {
    ...
    if (animator_update(&bar_manager->animator)) {
        ...
        if (bar_manager->bar_needs_resize)
...
void bar_manager_update(struct bar_manager *bar_manager, bool forced) {
    if ((bar_manager->frozen && !forced) || bar_manager->sleeps)
        return;
    ...
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        struct bar_item *bar_item = bar_manager->bar_items[i];
        needs_refresh |=
            bar_item_update(bar_item, NULL, forced, NULL, bar_manager->L);
    ...
    if (needs_refresh || forced)
        bar_manager_refresh(bar_manager, forced);
}
...
void bar_manager_begin(struct bar_manager *bar_manager) {
    if (bar_manager->displays == DISPLAY_MAIN_PATTERN) {
        ...
        bar_manager->bars = (struct bar **)realloc(
            bar_manager->bars, sizeof(struct bar *) * bar_manager->bar_count);

        memset(bar_manager->bars, 0,
        ...
        for (uint32_t index = 1; index <= display_count; index++) {
            if (!(bar_manager->displays & 1 << (index - 1)))
                continue;
            bar_count++;
        }
...
bool bar_manager_mouse_over_any_bar(struct bar_manager *bar_manager) {
    for (int i = 0; i < bar_manager->bar_count; i++) {
        if (bar_manager->bars[i]->mouse_over)
            return true;
    }
    return false;
}
...
void bar_manager_custom_events_trigger(struct bar_manager *bar_manager,
    ...
    uint64_t flag =
        custom_events_get_flag_for_name(&bar_manager->custom_events, name);

    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        struct bar_item *bar_item = bar_manager->bar_items[i];
        if (bar_item->update_mask & flag)
            bar_item_update(bar_item, name, false, env_vars, bar_manager->L);
    }
}

void bar_manager_display_resized(struct bar_manager *bar_manager,
                                 uint32_t did) {
    for (int i = 0; i < bar_manager->bar_count; i++) {
...
void bar_manager_display_removed(struct bar_manager *bar_manager,
                                 uint32_t did) {
...
void bar_manager_handle_mouse_scrolled_global(struct bar_manager *bar_manager,
    ...
    env_vars_set(&env_vars, string_copy("DID"), string_copy(adid_str));

    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_MOUSE_SCROLLED_GLOBAL, &env_vars);
...
void bar_manager_handle_mouse_entered(struct bar_manager *bar_manager,
    ...
    if (!bar_item)
        return;
    bar_item_mouse_entered(bar_item, bar_manager->L);
}
...
void bar_manager_handle_mouse_exited(struct bar_manager *bar_manager,
    ...
    if (!bar_item) {
        for (int i = 0; i < bar_manager->bar_item_count; i++)
            bar_item_mouse_exited(bar_manager->bar_items[i], bar_manager->L);
    } else {
        bar_item_mouse_exited(bar_item, bar_manager->L);
    }
}
...
void bar_manager_handle_volume_change(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_VOLUME_CHANGE, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_wifi_change(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(bar_manager,
                                      COMMAND_SUBSCRIBE_WIFI_CHANGE, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_brightness_change(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_BRIGHTNESS_CHANGE, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_power_source_change(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_POWER_SOURCE_CHANGE, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_media_change(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_MEDIA_CHANGE, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_front_app_switch(struct bar_manager *bar_manager,
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_FRONT_APP_SWITCHED, &env_vars);
    env_vars_destroy(&env_vars);
}
...
void bar_manager_handle_space_change(struct bar_manager *bar_manager,
    ...
    for (int i = 0; i < bar_manager->bar_count; i++) {
        ...
        bar_manager->needs_ordering |=
            !was_shown && bar_manager->bars[i]->shown;
        ...
        snprintf(info + cursor, 19 * bar_manager->bar_count + 4 - cursor,
                 ...
                 bar_manager->bars[i]->sid, separator);
        cursor = strlen(info);
    }
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_SPACE_CHANGE, &env_vars);
...
void bar_manager_handle_display_change(struct bar_manager *bar_manager) {
    ...
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_DISPLAY_CHANGE, &env_vars);
...
void bar_manager_handle_system_will_sleep(struct bar_manager *bar_manager) {
    bar_manager_custom_events_trigger(
        bar_manager, COMMAND_SUBSCRIBE_SYSTEM_WILL_SLEEP, NULL);
...
void bar_manager_handle_notification(struct bar_manager *bar_manager,
    ...
    char *name = custom_events_get_name_for_notification(
        &bar_manager->custom_events, notification->name);
    if (!name) {
...
void bar_manager_destroy(struct bar_manager *bar_manager) {
    ...
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        bar_item_destroy(bar_manager->bar_items[i], true, bar_manager->L);
    }
    if (bar_manager->bar_items)
        free(bar_manager->bar_items);
    for (int i = 0; i < bar_manager->bar_count; i++) {
        bar_destroy(bar_manager->bars[i]);
    }

    bar_item_destroy(&bar_manager->default_item, false, bar_manager->L);
    custom_events_destroy(&bar_manager->custom_events);
    background_destroy(&bar_manager->background);

    if (bar_manager->bars)
        free(bar_manager->bars);
    CFRunLoopRemoveTimer(CFRunLoopGetMain(), bar_manager->clock,
                         kCFRunLoopCommonModes);
...
void bar_manager_serialize(struct bar_manager *bar_manager, FILE *rsp) {
    ...
    fprintf(rsp,
            ...
            "%s\"blur_radius\": %u,\n"
            "%s\"margin\": %d,\n",
            indent, bar_manager->position == POSITION_BOTTOM ? "bottom" : "top",
            indent, format_bool(bar_manager->topmost), indent,
            format_bool(bar_manager->sticky), indent,
            format_bool(bar_manager->shadow), indent,
            format_bool(bar_manager->font_smoothing), indent,
            bar_manager->blur_radius, indent, bar_manager->margin);
    ...
    for (int i = 0; i < bar_manager->bar_item_count; i++) {
        ...
        if (i < bar_manager->bar_item_count - 1)
            fprintf(rsp, ",\n");
    }
    fprintf(rsp, "\n%s]\n}\n", indent);
}
