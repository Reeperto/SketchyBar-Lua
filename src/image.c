...
bool image_load(struct image *image, char *path, FILE *rsp) {
    ...
    if (app_kv.key && app_kv.value && strcmp(app_kv.key, "app") == 0) {
        ...
        CGDataProviderRef data_provider =
            CGDataProviderCreateWithFilename(res_path);
        if (strlen(res_path) > 3 &&
            string_equals(&res_path[strlen(res_path) - 4], ".png"))
            new_image_ref = CGImageCreateWithPNGDataProvider(
                data_provider, NULL, false, kCGRenderingIntentDefault);
        else {
            new_image_ref = CGImageCreateWithJPEGDataProvider(
                data_provider, NULL, false, kCGRenderingIntentDefault);
        }
    ...
    if (new_image_ref) {
        image_set_image(image, new_image_ref,
                        (CGRect){{0, 0},
    ...
    if (image->image_ref && image->data_ref) {
        ...
        if (old_len == new_len)
            equals = memcmp(CFDataGetBytePtr(image->data_ref),
                            CFDataGetBytePtr(new_data_ref), old_len) == 0;
    }
...
void image_copy(struct image *image, CGImageRef source) {
    if (source)
        image->image_ref = CGImageCreateCopy(source);
}

bool image_set_image(struct image *image, CGImageRef new_image_ref,
                     CGRect bounds, bool forced) {
    CFDataRef new_data_ref =
        CGDataProviderCopyData(CGImageGetDataProvider(new_image_ref));
    ...
    if (image->data_ref)
        CFRelease(image->data_ref);

    image->size = bounds.size;
    image->bounds = (CGRect){
        {0, 0},
        {bounds.size.width * image->scale, bounds.size.height * image->scale}};
...
bool image_set_scale(struct image *image, float scale) {
    ...
    image->scale = scale;
    image->bounds = (CGRect){
        {image->bounds.origin.x, image->bounds.origin.y},
...
void image_draw(struct image *image, CGContextRef context) {
    ...
    //   CGContextSetRGBFillColor(context, image->shadow.color.r,
    //   image->shadow.color.g, image->shadow.color.b, image->shadow.color.a);
...
void image_serialize(struct image *image, char *indent, FILE *rsp) {
    fprintf(rsp,
            ...
            "%s\"drawing\": \"%s\",\n"
            "%s\"scale\": %f",
            indent, image->path, indent, format_bool(image->enabled), indent,
            image->scale);
}

bool image_parse_sub_domain(struct image *image, FILE *rsp,
                            struct token property, char *message) {
    if (token_equals(property, PROPERTY_DRAWING))
        return image_set_enabled(
            image, evaluate_boolean_state(get_token(&message), image->enabled));
