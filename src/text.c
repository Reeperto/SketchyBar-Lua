...
static void text_prepare_line(struct text *text) {
    ...
    CFDictionaryRef attributes = CFDictionaryCreate(
        NULL, keys, values, array_count(keys), &kCFTypeDictionaryKeyCallBacks,
        &kCFTypeDictionaryValueCallBacks);

    CFStringRef string =
        CFStringCreateWithCString(NULL, text->string, kCFStringEncodingUTF8);

    if (!string)
        string = CFStringCreateWithCString(
            NULL, "Warning: Malformed UTF-8 string", kCFStringEncodingUTF8);

    CFAttributedStringRef attr_string =
        CFAttributedStringCreate(NULL, string, attributes);

    text->line.line = CTLineCreateWithAttributedString(attr_string);

    CTLineGetTypographicBounds(text->line.line, &text->line.ascent,
                               &text->line.descent, NULL);
...
bool text_set_string(struct text *text, char *string, bool forced) {
    ...
    if (string != text->string && text->string)
        free(text->string);
    text->string = string;
...
static bool text_set_width(struct text *text, int width) {
    ...
    if (text->custom_width == width && text->has_const_width)
        return false;
...
uint32_t text_get_length(struct text *text, bool override) {
    ...
    int len =
        text->bounds.size.width + text->padding_left + text->padding_right;
    if ((!text->has_const_width || override) && text->background.enabled &&
...
void text_calculate_bounds(struct text *text, uint32_t x, uint32_t y) {
    if (text->align == POSITION_CENTER && text->has_const_width)
        text->bounds.origin.x =
            (int)x +
            ((int)text->custom_width - (int)text_get_length(text, true)) / 2;
    else if (text->align == POSITION_RIGHT && text->has_const_width)
        text->bounds.origin.x =
            (int)x + (int)text->custom_width - (int)text_get_length(text, true);
    else
        text->bounds.origin.x = x;

    text->bounds.origin.y =
        (uint32_t)(y - ((text->line.ascent - text->line.descent) / 2));
    ...
    if (text->background.enabled) {
        ...
        background_calculate_bounds(&text->background, x, y,
                                    text_get_length(text, false), height);
    }
}
...
void text_draw(struct text *text, CGContextRef context) {
    ...
    if (text->shadow.enabled) {
        CGContextSetRGBFillColor(context, text->shadow.color.r,
                                 text->shadow.color.g, text->shadow.color.b,
                                 text->shadow.color.a);

        CGRect bounds = shadow_get_bounds(&text->shadow, text->bounds);
        CGContextSetTextPosition(context, bounds.origin.x + text->padding_left,
...
void text_serialize(struct text *text, char *indent, FILE *rsp) {
    ...
    fprintf(rsp,
            ...
            "%s\"align\": \"%s\",\n"
            "%s\"background\": {\n",
            indent, text->string, indent, format_bool(text->drawing), indent,
            format_bool(text->highlight), indent, text->color.hex, indent,
            text->highlight_color.hex, indent, text->padding_left, indent,
            text->padding_right, indent, text->y_offset, indent,
            text->font.family, text->font.style, text->font.size, indent,
            text->custom_width, indent, align, indent);
...
bool text_parse_sub_domain(struct text *text, FILE *rsp, struct token property,
    ...
    if (token_equals(property, PROPERTY_COLOR)) {
        ...
        ANIMATE_BYTES(text_set_color, text, text->color.hex,
                      token_to_int(token));
    } else if (token_equals(property, PROPERTY_HIGHLIGHT)) {
        bool highlight =
            evaluate_boolean_state(get_token(&message), text->highlight);
        if (g_bar_manager.animator.duration > 0) {
            if (text->highlight && !highlight) {
                ...
                ANIMATE_BYTES(text_set_color, text, text->color.hex, target);
            } else if (!text->highlight && highlight) {
                uint32_t target = text->highlight_color.hex;
                text_set_highlight_color(text, text->color.hex);

                ANIMATE_BYTES(text_set_highlight_color, text,
                              text->highlight_color.hex, target);
            }
        }
        ...
        struct token token = get_token(&message);
        ANIMATE_BYTES(text_set_highlight_color, text, text->highlight_color.hex,
                      token_to_int(token));
        ...
        struct token token = get_token(&message);
        ANIMATE(text_set_yoffset, text, text->y_offset, token_to_int(token));
        ...
        if (token_equals(token, ARGUMENT_DYNAMIC)) {
            ...
            animation_setup(animation, text,
                            (bool (*)(void *, int)) & text_set_width,
                            text->custom_width, -1, 1, INTERP_FUNCTION_LINEAR);
            animator_add(&g_bar_manager.animator, animation);
        } else {
            ANIMATE(text_set_width, text, text_get_length(text, false),
                    token_to_int(token));
        }
    } else if (token_equals(property, PROPERTY_DRAWING)) {
        bool prev = text->drawing;
        text->drawing =
            evaluate_boolean_state(get_token(&message), text->drawing);
        ...
        bool changed =
            text_set_string(text, token_to_string(get_token(&message)), false);

        if (changed && g_bar_manager.animator.duration > 0) {
            ...
            if (post_width != pre_width) {
                ...
                struct animation *animation = animation_create();
                animation_setup(
                    animation, text, (bool (*)(void *, int)) & text_set_width,
                    text->custom_width, -1, 1, INTERP_FUNCTION_LINEAR);
                animator_add(&g_bar_manager.animator, animation);
            }
        }
        ...
        if (key_value_pair.key && key_value_pair.value) {
            struct token subdom = {key_value_pair.key,
            ...
            if (token_equals(subdom, SUB_DOMAIN_BACKGROUND))
                return background_parse_sub_domain(&text->background, rsp,
                                                   entry, message);
            else if (token_equals(subdom, SUB_DOMAIN_SHADOW))
                return shadow_parse_sub_domain(&text->shadow, rsp, entry,
                                               message);
                ...
                return color_parse_sub_domain(&text->color, rsp, entry,
                                              message);
            else if (token_equals(subdom, SUB_DOMAIN_HIGHLIGHT_COLOR))
                return color_parse_sub_domain(&text->highlight_color, rsp,
                                              entry, message);
            else
