...
void shadow_init(struct shadow *shadow) {
    ...
    shadow->distance = 5;
    shadow->offset.x =
        ((float)shadow->distance) * cos(((double)shadow->angle) * deg_to_rad);
    shadow->offset.y =
        -((float)shadow->distance) * sin(((double)shadow->angle) * deg_to_rad);
...
void shadow_serialize(struct shadow *shadow, char *indent, FILE *rsp) {
    fprintf(rsp,
            ...
            "%s\"angle\": %u,\n"
            "%s\"distance\": %u",
            indent, format_bool(shadow->enabled), indent, shadow->color.hex,
            indent, shadow->angle, indent, shadow->distance);
}

bool shadow_parse_sub_domain(struct shadow *shadow, FILE *rsp,
                             struct token property, char *message) {
    bool needs_refresh = false;
    if (token_equals(property, PROPERTY_DRAWING)) {
        needs_refresh = shadow_set_enabled(
            shadow,
            evaluate_boolean_state(get_token(&message), shadow->enabled));
        ...
        struct token token = get_token(&message);
        ANIMATE(shadow_set_angle, shadow, shadow->angle, token_to_int(token));
    } else if (token_equals(property, PROPERTY_COLOR)) {
        struct token token = get_token(&message);
        ANIMATE_BYTES(shadow_set_color, shadow, shadow->color.hex,
                      token_to_int(token));
    } else {
        ...
        if (key_value_pair.key && key_value_pair.value) {
            ...
            if (token_equals(subdom, SUB_DOMAIN_COLOR)) {
                return color_parse_sub_domain(&shadow->color, rsp, entry,
                                              message);
            } else {
