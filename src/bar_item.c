...
void bar_item_init(struct bar_item *bar_item, struct bar_item *default_item) {
    ...
    if (default_item)
        bar_item_inherit_from_item(bar_item, default_item);
}

void bar_item_append_associated_space(struct bar_item *bar_item, uint32_t bit) {
    if (bar_item->associated_space & bit)
        return;
    ...
    if (bar_item->type == BAR_COMPONENT_SPACE) {
        ...
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("SID"),
                     string_copy(sid_str));
    }
}

void bar_item_append_associated_display(struct bar_item *bar_item,
                                        uint32_t bit) {
    if (bar_item->associated_display & bit)
        return;
    ...
    if (bar_item->type == BAR_COMPONENT_SPACE) {
        ...
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("DID"),
                     string_copy(did_str));
    }
}

bool bar_item_is_shown(struct bar_item *bar_item) {
    if (bar_item->associated_bar & UINT32_MAX)
        return true;
    else
        return false;
}
...
bool bar_item_update(struct bar_item *bar_item, char *sender, bool forced,
                     struct env_vars *env_vars, lua_State *L) {
    ...
    bool scheduled_update_needed =
        bar_item->update_frequency <= bar_item->counter;
    bool should_update =
        bar_item->updates_only_when_shown ? bar_item_is_shown(bar_item) : true;
    ...
    if (((scheduled_update_needed || sender) && should_update) || forced) {
            ...
            if (!env_vars)
                ...
                for (int i = 0; i < bar_item->signal_args.env_vars.count; i++) {
                    env_vars_set(
                        env_vars,
                        string_copy(
                            bar_item->signal_args.env_vars.vars[i]->key),
                        string_copy(
                            bar_item->signal_args.env_vars.vars[i]->value));
                }
                env_vars_set(env_vars, string_copy("NAME"),
                             string_copy(bar_item->name));
            }

            if (sender)
                env_vars_set(env_vars, string_copy("SENDER"),
                             string_copy(sender));
            else
                env_vars_set(env_vars, string_copy("SENDER"),
        ...
        if (bar_item->event_port) {
            uint32_t len = 0;
            char *message =
                env_vars_copy_serialized_representation(env_vars, &len);
...
void bar_item_cancel_drag(struct bar_item *bar_item) {
    if (bar_item->has_slider) {
        ...
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("PERCENTAGE"),
...
void bar_item_on_click(struct bar_item *bar_item, uint32_t type,
                       uint32_t mouse_button_code, uint32_t modifier,
                       CGPoint point, lua_State *L) {
    if (!bar_item)
        return;
    ...
    snprintf(info_str, 256,
             ...
             "\t\"modfier_code\": %u\n"
             "}\n",
             get_type_description(type), mouse_button_code,
             get_modifier_description(modifier), modifier);

    env_vars_set(&env_vars, string_copy("INFO"), string_copy(info_str));

    env_vars_set(&env_vars, string_copy("BUTTON"),
                 string_copy(get_type_description(type)));

    env_vars_set(&env_vars, string_copy("MODIFIER"),
...
void bar_item_on_scroll(struct bar_item *bar_item, int scroll_delta,
    ...
    env_vars_set(&env_vars, string_copy("SCROLL_DELTA"),
                 string_copy(delta_ver_str));

    if (bar_item->update_mask & UPDATE_MOUSE_SCROLLED)
        bar_item_update(bar_item, COMMAND_SUBSCRIBE_MOUSE_SCROLLED, true,
                        &env_vars, L);

    env_vars_destroy(&env_vars);
}

void bar_item_mouse_entered(struct bar_item *bar_item, lua_State *L) {
    if (bar_item->update_mask & UPDATE_MOUSE_ENTERED)
        bar_item_update(bar_item, COMMAND_SUBSCRIBE_MOUSE_ENTERED, true, NULL,
                        L);
}

void bar_item_mouse_exited(struct bar_item *bar_item, lua_State *L) {
...
static bool bar_item_set_yoffset(struct bar_item *bar_item, int offset) {
    if (bar_item->y_offset == offset)
        return false;
    bar_item->y_offset = offset;
    return true;
}

static bool bar_item_set_blur_radius(struct bar_item *bar_item,
                                     uint32_t radius) {
    if (bar_item->blur_radius == radius)
        return false;
    bar_item->blur_radius = radius;
    for (int i = 0; i < bar_item->num_windows; i++) {
        if (!bar_item->windows[i])
            continue;
...
bool bar_item_set_name(struct bar_item *bar_item, char *name) {
    ...
    if (name != bar_item->name && bar_item->name)
        free(bar_item->name);
    bar_item->name = name;
    env_vars_set(&bar_item->signal_args.env_vars, string_copy("NAME"),
                 string_copy(name));
    return true;
}
...
bool bar_item_set_type(struct bar_item *bar_item, char *type) {
    ...
    if (bar_item->type == BAR_COMPONENT_SPACE) {
        ...
        bar_item->updates_only_when_shown = false;
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("SELECTED"),
                     string_copy("false"));
...
static uint32_t bar_item_get_content_length(struct bar_item *bar_item) {
    int length =
        text_get_length(&bar_item->icon, false) +
...
uint32_t bar_item_get_length(struct bar_item *bar_item, bool ignore_override) {
    ...
    if (bar_item->has_const_width &&
        (!ignore_override || bar_item->custom_width > content_length)) {
...
uint32_t bar_item_get_height(struct bar_item *bar_item) {
    ...
    if (bar_item->background.enabled) {
        uint32_t image_height =
            bar_item->background.image.enabled
                ? bar_item->background.image.bounds.size.height
                : 0;

        background_height =
            max(image_height, bar_item->background.bounds.size.height);
    }
...
struct window *bar_item_get_window(struct bar_item *bar_item, uint32_t adid) {
    ...
    if (bar_item->num_windows < adid) {
        bar_item->windows = (struct window **)realloc(
            bar_item->windows, sizeof(struct window *) * adid);
        memset(bar_item->windows + bar_item->num_windows, 0,
    ...
    if (!bar_item->windows[adid - 1]) {
        ...
        window_set_blur_radius(bar_item->windows[adid - 1],
...
uint32_t bar_item_calculate_bounds(struct bar_item *bar_item,
                                   uint32_t bar_height, uint32_t x,
                                   uint32_t y) {
    ...
    text_calculate_bounds(&bar_item->icon, icon_position,
                          content_y + bar_item->y_offset);

    text_calculate_bounds(&bar_item->label, label_position,
                          content_y + bar_item->y_offset);

    if (bar_item->has_alias)
        alias_calculate_bounds(&bar_item->alias, sandwich_position,
                               content_y + bar_item->y_offset);

    if (bar_item->has_slider)
        slider_calculate_bounds(&bar_item->slider, sandwich_position,
                                content_y + bar_item->y_offset);
    ...
    if (bar_item->has_graph) {
        ...
        graph_calculate_bounds(&bar_item->graph, sandwich_position,
                               content_y + bar_item->y_offset, height);
    }

    if (bar_item->background.enabled) {
        uint32_t height =
            bar_item->background.overrides_height
                ? bar_item->background.bounds.size.height
                : (bar_height - (g_bar_manager.background.border_width + 1));

        background_calculate_bounds(&bar_item->background, x,
                                    content_y + bar_item->y_offset,
                                    bar_item_length, height);
    }

    return bar_item_length;
}

bool bar_item_clip_needs_update_for_bar(struct bar_item *bar_item,
                                        struct bar *bar) {
    bool needs_update = false;

    needs_update |=
        background_clip_needs_update(&bar_item->background, bar) ||
...
void bar_item_change_space(struct bar_item *bar_item, uint64_t dsid,
                           uint32_t adid) {
    if (bar_item->num_windows >= adid && bar_item->windows[adid - 1]) {
...
void bar_item_inherit_from_item(struct bar_item *bar_item,
                                struct bar_item *ancestor) {
    ...
    if (bar_item->type == BAR_COMPONENT_SPACE) {
        ...
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("SID"),
                     string_copy(env_vars_get_value_for_key(
                         &ancestor->signal_args.env_vars, "DID")));
        env_vars_set(&bar_item->signal_args.env_vars, string_copy("DID"),
                     string_copy(env_vars_get_value_for_key(
                         &ancestor->signal_args.env_vars, "DID")));
    }
...
void bar_item_serialize(struct bar_item *bar_item, FILE *rsp) {
    ...
    fprintf(rsp,
            ...
            "\t\t\"padding_right\": %d,\n"
            "\t\t\"background\": {\n",
            bar_item->name, type, format_bool(bar_item->drawing), position,
            bar_item->associated_space, bar_item->associated_display,
            format_bool(bar_item->ignore_association), bar_item->y_offset,
    ...
    for (int i = 0; i < bar_item->num_windows; i++) {
        ...
        fprintf(rsp,
                ...
                "\t\t\t\"origin\": [ %f, %f ],\n"
                "\t\t\t\"size\": [ %f, %f ]\n\t\t}",
                i + 1, bar_item->windows[i]->origin.x,
...
void bar_item_parse_set_message(struct bar_item *bar_item, char *message,
                                FILE *rsp) {
    ...
    if (key_value_pair.key && key_value_pair.value) {
        ...
        if (token_equals(subdom, SUB_DOMAIN_ICON)) {
            needs_refresh =
                text_parse_sub_domain(&bar_item->icon, rsp, entry, message);
        } else if (token_equals(subdom, SUB_DOMAIN_LABEL)) {
            needs_refresh =
                text_parse_sub_domain(&bar_item->label, rsp, entry, message);
            ...
            needs_refresh =
                popup_parse_sub_domain(&bar_item->popup, rsp, entry, message);
        } else if (token_equals(subdom, SUB_DOMAIN_GRAPH)) {
            if (bar_item->has_graph ||
                bar_item == &g_bar_manager.default_item) {
                needs_refresh = graph_parse_sub_domain(&bar_item->graph, rsp,
                                                       entry, message);
            } else {
                respond(rsp,
                        "[!] Item (%s): Trying to set a graph property on a "
                        "non-graph "
                        "item\n",
                        bar_item->name);
            }
            ...
            if (bar_item->has_alias ||
                bar_item == &g_bar_manager.default_item) {
                needs_refresh = alias_parse_sub_domain(&bar_item->alias, rsp,
                                                       entry, message);
            } else {
                respond(rsp,
                        "[!] Item (%s): Trying to set an alias property on a "
                        "non-alias "
                        "item\n",
                        bar_item->name);
            }
            ...
            if (bar_item->has_slider ||
                bar_item == &g_bar_manager.default_item) {
                needs_refresh = slider_parse_sub_domain(&bar_item->slider, rsp,
                                                        entry, message);
            } else {
                respond(rsp,
                        "[!] Item (%s): Trying to set a slider property on a "
                        "non-slider item\n",
                        bar_item->name);
            }
        } else {
        ...
        needs_refresh =
            text_parse_sub_domain(&bar_item->icon, rsp, dummy, message);

    } else if (token_equals(property, PROPERTY_LABEL)) {
        struct token dummy = {PROPERTY_STRING, strlen(PROPERTY_STRING)};
        needs_refresh =
            text_parse_sub_domain(&bar_item->label, rsp, dummy, message);
        ...
        if (token_equals(token, ARGUMENT_UPDATES_WHEN_SHOWN)) {
            ...
            bar_item->updates =
                evaluate_boolean_state(token, bar_item->updates);
            bar_item->updates_only_when_shown = false;
        }
    } else if (token_equals(property, PROPERTY_DRAWING)) {
        needs_refresh = bar_item_set_drawing(
            bar_item,
            evaluate_boolean_state(get_token(&message), bar_item->drawing));
    ...
    // }
        ...
        struct key_value_pair key_value_pair =
            get_key_value_pair(position.text, '.');

        if (key_value_pair.key && key_value_pair.value) {
            if (key_value_pair.key[0] == POSITION_POPUP) {
                int item_index_for_name = bar_manager_get_item_index_for_name(
                    &g_bar_manager, key_value_pair.value);
                if (item_index_for_name < 0) {
                    respond(rsp,
                            "[!] Item Position (%s): Item '%s' is not a valid "
                            "popup host\n",
                            bar_item->name, key_value_pair.value);
                    return;
                }
                struct bar_item *target_item =
                    g_bar_manager.bar_items[item_index_for_name];
        ...
        if (list && count > 0) {
            for (int i = 0; i < count; i++) {
                bar_item_append_associated_space(
                    bar_item, 1 << strtoul(list[i], NULL, 0));
            }
            free(list);
        }
        ...
        if (list && count > 0) {
            for (int i = 0; i < count; i++) {
                if (strcmp(list[i], "active") == 0) {
                    ...
                    bar_item_append_associated_display(
                        bar_item, 1 << strtoul(list[i], NULL, 0));
                }
            }
            free(list);
        ...
        struct token token = get_token(&message);
        ANIMATE(background_set_padding_left, &bar_item->background,
                bar_item->background.padding_left, token_to_int(token));

    } else if (token_equals(property, PROPERTY_PADDING_RIGHT)) {
        struct token token = get_token(&message);
        ANIMATE(background_set_padding_right, &bar_item->background,
                bar_item->background.padding_right, token_to_int(token));

    } else if (token_equals(property, PROPERTY_BLUR_RADIUS)) {
        struct token token = get_token(&message);
        ANIMATE(bar_item_set_blur_radius, bar_item, bar_item->blur_radius,
                token_to_int(token));

    } else if (token_equals(property, PROPERTY_SHADOW)) {
        bool prev = bar_item->shadow;
        bar_item->shadow =
            evaluate_boolean_state(get_token(&message), bar_item->shadow);
        ...
        bar_item->ignore_association = evaluate_boolean_state(
            get_token(&message), bar_item->ignore_association);
        needs_refresh = true;
...
void bar_item_parse_subscribe_message(struct bar_item *bar_item, char *message,
    ...
    while (event.text && event.length > 0) {
        uint64_t event_flag = custom_events_get_flag_for_name(
            &g_bar_manager.custom_events, event.text);
