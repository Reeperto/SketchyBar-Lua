...
#define ANIMATE(f, o, p, t)                                                    \
    {                                                                          \
        if (g_bar_manager.animator.duration > 0) {                             \
            animator_cancel_locked(&g_bar_manager.animator, (void *)o,         \
                                   (bool (*)(void *, int)) & f);               \
            struct animation *animation = animation_create();                  \
            animation_setup(animation, (void *)o, (bool (*)(void *, int)) & f, \
                            p, t, g_bar_manager.animator.duration,             \
                            g_bar_manager.animator.interp_function);           \
            animator_add(&g_bar_manager.animator, animation);                  \
        } else {                                                               \
            needs_refresh =                                                    \
                animator_cancel(&g_bar_manager.animator, (void *)o,            \
                                (bool (*)(void *, int)) & f);                  \
            needs_refresh |= f(o, t);                                          \
        }                                                                      \
    }
...
#define ANIMATE_FLOAT(f, o, p, t)                                              \
    {                                                                          \
        if (g_bar_manager.animator.duration > 0) {                             \
            ...
            float final_value = t;                                             \
            animation_setup(animation, (void *)o, (bool (*)(void *, int)) & f, \
                            *(int *)&initial_value, *(int *)&final_value,      \
...
#define ANIMATOR_CALLBACK(name)                                                \
    void name(CFRunLoopTimerRef timer, void *context)
