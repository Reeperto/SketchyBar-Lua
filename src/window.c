...
void window_create(struct window *window, CGRect frame) {
    ...
    uint64_t id;
    SLSNewWindow(g_connection, kCGBackingStoreBuffered, window->origin.x,
                 window->origin.y, frame_region, &id);
...
void window_move(struct window *window, CGPoint point) {
    ...
    if (__builtin_available(macOS 12.0, *)) {
        ...
        CFNumberRef number =
            CFNumberCreate(NULL, kCFNumberSInt32Type, &window->id);
...
bool window_apply_frame(struct window *window, bool forced) {
    ...
    if (window->needs_resize) {
        ...
        if (__builtin_available(macOS 13.0, *)) {
            SLSSetWindowShape(g_connection, window->id, g_nirvana.x,
                              g_nirvana.y, frame_region);
        } else {
            if (window->parent) {
...
void window_send_to_space(struct window *window, uint64_t dsid) {
    CFArrayRef window_list = cfarray_of_cfnumbers(&window->id, sizeof(uint32_t),
...
void window_disable_shadow(struct window *window) {
    ...
    CFNumberRef shadow_density_cf = CFNumberCreate(
        kCFAllocatorDefault, kCFNumberCFIndexType, &shadow_density);

    const void *keys[1] = {CFSTR("com.apple.WindowShadowDensity")};
    const void *values[1] = {shadow_density_cf};
    CFDictionaryRef shadow_props_cf = CFDictionaryCreate(
...
CGImageRef window_capture(struct window *window, bool *disabled) {
    ...
    *disabled = false;
    ...
    SLSCaptureWindowsContentsToRectWithOptions(g_connection, &wid, true,
                                               CGRectNull, 1 << 8, &image_ref);
