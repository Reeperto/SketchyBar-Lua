...
static bool slider_set_width(struct slider *slider, uint32_t width) {
    if (width == slider->background.bounds.size.width)
        return false;
...
static bool slider_set_foreground_color(struct slider *slider, uint32_t color) {
    if (slider->foreground_color == color)
        return false;
...
static bool slider_set_percentage(struct slider *slider, uint32_t percentage) {
    if (percentage == slider->percentage)
        return false;
...
uint32_t slider_get_percentage_for_point(struct slider *slider, CGPoint point) {
    ...
    if (delta < 0)
        delta = 0;
    uint32_t percentage =
        delta / slider->background.bounds.size.width * 100.f + 0.5f;

    return min(percentage, 100);
}

void slider_cancel_drag(struct slider *slider) { slider->is_dragged = false; }
...
void slider_calculate_bounds(struct slider *slider, uint32_t x, uint32_t y) {
    background_calculate_bounds(&slider->background, x, y,
                                slider->background.bounds.size.width,
                                slider->background.bounds.size.height);

    background_calculate_bounds(&slider->foreground, x, y,
                                slider->background.bounds.size.width *
    ...
    uint32_t knob_offset =
        max(min(raw_offset, slider->background.bounds.size.width -
                                (slider->knob.bounds.size.width + 1.f)),
            0.f);
...
void slider_serialize(struct slider *slider, char *indent, FILE *rsp) {
    fprintf(rsp,
            ...
            "%s\"percentage\": \"%d\",\n"
            "%s\"width\": \"%d\",\n",
            indent, slider->foreground_color, indent, slider->percentage,
...
bool slider_parse_sub_domain(struct slider *slider, FILE *rsp,
                             struct token property, char *message) {
    bool needs_refresh = false;
    if (token_equals(property, PROPERTY_PERCENTAGE)) {
        struct token token = get_token(&message);
        if (!slider->is_dragged) {
            ANIMATE(slider_set_percentage, slider, slider->percentage,
                    token_to_uint32t(token));
        }
    } else if (token_equals(property, PROPERTY_HIGHLIGHT_COLOR)) {
        struct token token = get_token(&message);
        ANIMATE_BYTES(slider_set_foreground_color, slider,
                      slider->foreground_color, token_to_uint32t(token));
    } else if (token_equals(property, PROPERTY_WIDTH)) {
        struct token token = get_token(&message);
        if (!slider->is_dragged) {
            ANIMATE(slider_set_width, slider,
                    slider->background.bounds.size.width,
                    token_to_uint32t(token));
        }
        ...
        needs_refresh =
            text_parse_sub_domain(&slider->knob, rsp, dummy, message);
    } else {
        ...
        if (key_value_pair.key && key_value_pair.value) {
            ...
            if (token_equals(subdom, SUB_DOMAIN_BACKGROUND)) {
                background_parse_sub_domain(&slider->foreground, rsp, entry,
                                            message);
                background_set_color(&slider->foreground,
                                     slider->foreground_color);
                return background_parse_sub_domain(&slider->background, rsp,
                                                   entry, message);
            } else if (token_equals(subdom, SUB_DOMAIN_KNOB))
                return text_parse_sub_domain(&slider->knob, rsp, entry,
                                             message);
            else {
