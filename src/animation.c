...
void animation_setup(struct animation *animation, void *target,
                     animator_function *update_function, int initial_value,
                     int final_value, uint32_t duration, char interp_function) {
    animation->counter = 1;
...
static bool animation_update(struct animation *animation) {
    ...
    if (animation->separate_bytes) {
        for (int i = 0; i < 4; i++) {
            unsigned char byte_i =
                *((unsigned char *)&animation->initial_value + i);
            unsigned char byte_f =
                *((unsigned char *)&animation->final_value + i);
        ...
        *((float *)&value) =
            (1. - slider) * *(float *)&animation->initial_value +
    ...
    for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
        if (needs_update &&
            ...
            (animation->target <
             ((void *)g_bar_manager.bar_items[i] + sizeof(struct bar_item)))) {
...
static void
animator_calculate_offset_for_animation(struct animator *animator,
...
void animator_add(struct animator *animator, struct animation *animation) {
    ...
    if (animator->animation_count == 1) {
        animator->clock =
            CFRunLoopTimerCreate(NULL, CFAbsoluteTimeGetCurrent() + 1. / 60.,
                                 1. / 60., 0, 0, animator_handler, NULL);

        CFRunLoopAddTimer(CFRunLoopGetMain(), animator->clock,
                          kCFRunLoopCommonModes);
    }
}

static void animator_remove(struct animator *animator,
                            struct animation *animation) {
    if (animator->animation_count == 1) {
        ...
        memcpy(animator->animations, tmp,
               sizeof(struct animation *) * animator->animation_count);
    }
    ...
    if (animator->animation_count == 0) {
        CFRunLoopRemoveTimer(CFRunLoopGetMain(), animator->clock,
                             kCFRunLoopCommonModes);
        CFRelease(animator->clock);
    }
}

void animator_cancel_locked(struct animator *animator, void *target,
                            animator_function *function) {
    ...
    for (int i = 0; i < animator->animation_count; i++) {
        ...
        if (animation->locked && animation->target == target &&
...
bool animator_cancel(struct animator *animator, void *target,
                     animator_function *function) {
...
bool animator_update(struct animator *animator) {
    ...
    for (uint32_t i = 0; i < animator->animation_count; i++) {
        ...
        if (animator->animations[i]->counter >
            animator->animations[i]->duration) {
