...
static void event_mach_message(void *context) { handle_message_mach(context); }
...
static void event_mouse_up(void *context) {
    ...
    struct bar_item *bar_item =
        bar_manager_get_item_by_wid(&g_bar_manager, wid, adid);
    ...
    bar_item_on_click(bar_item, type, mouse_button_code, modifier_keys,
                      point_in_window_coords, g_bar_manager.L);
...
static void event_mouse_exited(void *context) {
    ...
    if ((bar = bar_manager_get_bar_by_wid(&g_bar_manager, wid))) {
        ...
        popup_target = bar_manager_get_popup_by_point(&g_bar_manager, point);
...
static void event_mouse_scrolled(void *context) {
    ...
    uint32_t wid = get_wid_from_cg_event(context);
    int scroll_delta =
        CGEventGetIntegerValueField(context, kCGScrollWheelEventDeltaAxis1);
    uint32_t adid = display_arrangement(display_active_display_id());
    ...
    if (bar) {
        ...
        if (bar->mouse_over &&
            ...
            bar_manager_handle_mouse_scrolled_global(
                &g_bar_manager, scroll_delta + g_scroll_info.delta_y,
                bar->adid);
        }
