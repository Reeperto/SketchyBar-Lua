...
void mach_receive_message(mach_port_t port, struct mach_buffer *buffer,
    ...
    *buffer = (struct mach_buffer){0};
    ...
    if (timeout)
        msg_return =
            mach_msg(&buffer->message.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT,
                     0, sizeof(struct mach_buffer), port, 100, MACH_PORT_NULL);
    else
...
char *mach_send_message(mach_port_t port, char *message, uint32_t len,
    ...
    if (await_response) {
        ...
        msg.header.msgh_bits = MACH_MSGH_BITS_SET(
            MACH_MSG_TYPE_COPY_SEND & MACH_MSGH_BITS_REMOTE_MASK, 0, 0,
...
void mach_message_callback(CFMachPortRef port, void *message, CFIndex size,
                           void *context) {
...
bool mach_server_begin(struct mach_server *mach_server, mach_handler handler) {
  ...
  if (mach_port_insert_right(mach_server->task,
    ...
    if (bootstrap_register(mach_server->bs_port, bs_name, mach_server->port) !=
        KERN_SUCCESS) {
        return false;
    }
    ...
    CFMachPortRef cf_mach_port = CFMachPortCreateWithPort(
        NULL, mach_server->port, mach_message_callback, &context, false);
