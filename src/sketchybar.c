...
static int client_send_message(int argc, char **argv) {
    ...
    *temp++ = '\0';
    ...
    char *rsp = mach_send_message(mach_get_bs_port(bs_name), message,
                                  message_length, true);
...
static void parse_arguments(int argc, char **argv) {
    if ((string_equals(argv[1], VERSION_OPT_LONG)) ||
        ...
        if (argc < 3) {
            ...
            if (set_config_file_path(argv[2]))
                return;
...
void system_events(uint32_t event, void *data, size_t data_length,
                   void *context) {
    if (event == 1322) {
...
void mach_port_callback(CFMachPortRef port, void *message, CFIndex size,
                        void *context) {
    ...
    do {
        ...
        if (type != 0xd)
            mouse_handle_event(type, cg_event);
...
int main(int argc, char **argv) {
    ...
    if (error == kCGErrorSuccess) {
        cf_mach_port = CFMachPortCreateWithPort(NULL, port, mach_port_callback,
                                                NULL, false);
