...
static struct bar_item *group_get_first_member(struct group *group,
                                               struct bar *bar) {
...
static struct bar_item *group_get_last_member(struct group *group,
                                              struct bar *bar) {
    if (group->num_members == 1)
        return NULL;
...
void group_add_member(struct group *group, struct bar_item *item) {
    if (group_is_item_member(group, item))
        return;
    if (item->group && item->group->members &&
        item->group->members[0] == item) {
        for (int i = 1; i < item->group->num_members; i++) {
            group_add_member(group, item->group->members[i]);
        }
    } else {
        group->num_members++;
        group->members = realloc(group->members, sizeof(struct bar_item *) *
                                                     group->num_members);
...
void group_remove_member(struct group *group, struct bar_item *bar_item) {
    ...
    for (int i = 0; i < group->num_members; i++) {
        if (group->members[i] == bar_item)
            continue;
        tmp[count++] = group->members[i];
    }
    group->num_members--;
    group->members =
        realloc(group->members, sizeof(struct bar_item *) * group->num_members);
...
void group_calculate_bounds(struct group *group, struct bar *bar, uint32_t y) {
    ...
    CGPoint shadow_offsets =
        bar_item_calculate_shadow_offsets(group->members[0]);

    group->bounds =
        (CGRect){{group->first_window->origin.x -
                      group->first_item->background.padding_left,
    ...
    background_calculate_bounds(
        &group->members[0]->background, max(shadow_offsets.x, 0),
        y + group->members[0]->y_offset, group_get_length(group, bar),
