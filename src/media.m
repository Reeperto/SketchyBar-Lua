...
- (void)update {
    @autoreleasepool {
        if (self.app && self.artist && self.title && self.album) {
            ...
            uint32_t info_len = strlen(app_cstr) + strlen(artist_cstr) +
                                strlen(title_cstr) + strlen(album_cstr) + 256;
            ...
            snprintf(info, info_len,
                     ...
                     "\t\"artist\": \"%s\",\n"
                     "\t\"app\": \"%s\"\n}",
                     self.playing ? "playing" : "paused", title_cstr,
                     album_cstr, artist_cstr, app_cstr);
...
- (void)playing_change:(NSNotification *)notification {
    ...
    MRMediaRemoteGetNowPlayingApplicationIsPlaying(
        dispatch_get_main_queue(), ^(BOOL playing) {
...
void begin_receiving_media_events() { g_media_events = true; }
