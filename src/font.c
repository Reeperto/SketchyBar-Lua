...
void font_create_ctfont(struct font *font) {
    ...
    CFStringRef family_ref =
        CFStringCreateWithCString(NULL, font->family, kCFStringEncodingUTF8);

    CFStringRef style_ref =
        CFStringCreateWithCString(NULL, font->style, kCFStringEncodingUTF8);

    CFNumberRef size_ref =
        CFNumberCreate(NULL, kCFNumberFloat32Type, &font->size);
...
bool font_set_style(struct font *font, char *style, bool forced) {
    ...
    if (font->style && style != font->style)
        free(font->style);
    font->style = style;
...
bool font_set(struct font *font, char *font_string, bool forced) {
    ...
    sscanf(font_string, "%254[^:]:%254[^:]:%f", font_properties[0],
           font_properties[1], &size);

    free(font_string);

    bool change =
        font_set_family(font, string_copy(font_properties[0]), forced);
...
bool font_parse_sub_domain(struct font *font, FILE *rsp, struct token property,
                           char *message) {
    bool needs_refresh = false;
    if (token_equals(property, PROPERTY_FONT_SIZE)) {
        struct token token = get_token(&message);
        ANIMATE_FLOAT(font_set_size, font, font->size, token_to_float(token));
