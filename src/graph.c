...
void graph_calculate_bounds(struct graph *graph, uint32_t x, uint32_t y,
    ...
    graph->bounds.origin.y =
        y - graph->bounds.size.height / 2 + graph->line_width;
}
...
void graph_draw(struct graph *graph, CGContextRef context) {
    ...
    CGContextSetRGBStrokeColor(context, graph->line_color.r,
                               graph->line_color.g, graph->line_color.b,
                               graph->line_color.a);

    if (graph->overrides_fill_color)
        CGContextSetRGBFillColor(context, graph->fill_color.r,
                                 graph->fill_color.g, graph->fill_color.b,
                                 graph->fill_color.a);
    else
        CGContextSetRGBFillColor(context, graph->line_color.r,
                                 graph->line_color.g, graph->line_color.b,
...
void graph_serialize(struct graph *graph, char *indent, FILE *rsp) {
    fprintf(rsp,
            ...
            "%s\"line_width\": \"%f\",\n"
            "%s\"data\": [\n",
            indent, graph->line_color.hex, indent, graph->fill_color.hex,
...
bool graph_parse_sub_domain(struct graph *graph, FILE *rsp,
