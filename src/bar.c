    ...
    if (!g_bar_manager.bar_needs_update) {
        for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
            ...
            if (!clips_bar || (!bar_draws_item(bar, bar_item))) {
                ...
                if (clips_bar &&
                    !CGPointEqualToPoint(window->origin, g_nirvana)) {
...
void bar_draw(struct bar *bar, bool forced) {
    ...
    for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
        ...
        if (g_bar_manager.bar_needs_update) {
            bar_item_clip_bar(bar_item, window->origin.x - bar->window.origin.x,
                              bar);
        }
...
static void bar_calculate_bounds_left_right(struct bar *bar) {
    ...
    for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
        ...
        CGRect frame = {{bar->window.origin.x + x - max(shadow_offsets.x, 0),
                         bar->window.origin.y + *next_position +
...
static void bar_create_window(struct bar *bar) {
    ...
    context_set_font_smoothing(bar->window.context,
