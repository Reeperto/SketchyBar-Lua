...
bool background_set_clip(struct background *background, float clip) {
...
bool background_set_border_color(struct background *background,
                                        uint32_t color) {
    return color_set_hex(&background->border_color, color);
}

bool background_set_border_width(struct background *background,
...
bool background_set_corner_radius(struct background *background,
...
static void draw_rect(CGContextRef context, CGRect region,
    ...
    CGContextSetRGBFillColor(context, fill_color->r, fill_color->g,
                             fill_color->b, fill_color->a);

    CGMutablePathRef path = CGPathCreateMutable();
    CGRect inset_region = CGRectInset(region, (float)(line_width) / 2.f,
    ...
    CGPathAddRoundedRect(path, NULL, inset_region, corner_radius,
                         corner_radius);
...
void background_draw(struct background *background, CGContextRef context) {
    ...
    if (background->shadow.enabled) {
        CGRect bounds =
            shadow_get_bounds(&background->shadow, background_bounds);
...
bool background_parse_sub_domain(struct background *background, FILE *rsp,
    ...
    if (token_equals(property, PROPERTY_DRAWING))
        ...
        return image_load(&background->image,
                          token_to_string(get_token(&message)), rsp);
    } else {
        ...
        if (key_value_pair.key && key_value_pair.value) {
            ...
            if (token_equals(subdom, SUB_DOMAIN_SHADOW))
                ...
                return image_parse_sub_domain(&background->image, rsp, entry,
                                              message);
            } else if (token_equals(subdom, SUB_DOMAIN_COLOR)) {
                return color_parse_sub_domain(&background->color, rsp, entry,
                                              message);
            } else if (token_equals(subdom, SUB_DOMAIN_BORDER_COLOR)) {
                return color_parse_sub_domain(&background->border_color, rsp,
                                              entry, message);
            } else {
