...
static OSStatus handler(AudioObjectID id, uint32_t address_count,
    ...
    AudioObjectGetPropertyData(id, &kMuteMainPropertyAddress, 0, NULL, &size,
                               &muted_main);

    uint32_t muted_left = 0;
    size = sizeof(uint32_t);
    AudioObjectGetPropertyData(id, &kMuteLeftPropertyAddress, 0, NULL, &size,
                               &muted_left);

    size = sizeof(float);
    float volume_main = 0.f;
    AudioObjectGetPropertyData(id, &kVolumeMainPropertyAddress, 0, NULL, &size,
                               &volume_main);

    size = sizeof(float);
    float volume_left = 0.f;
    AudioObjectGetPropertyData(id, &kVolumeLeftPropertyAddress, 0, NULL, &size,
                               &volume_left);
...
OSStatus device_changed(AudioObjectID id, uint32_t address_count,
                        const AudioObjectPropertyAddress *addresses,
                        void *context) {
    AudioObjectID new_id = 0;
    uint32_t size = sizeof(AudioObjectID);
    AudioObjectGetPropertyData(kAudioObjectSystemObject,
                               &kHardwareDevicePropertyAddress, 0, NULL, &size,
                               &new_id);

    if (g_audio_id) {
        AudioObjectRemovePropertyListener(g_audio_id, &kMuteMainPropertyAddress,
                                          &handler, NULL);

        AudioObjectRemovePropertyListener(g_audio_id, &kMuteLeftPropertyAddress,
                                          &handler, NULL);

        AudioObjectRemovePropertyListener(
            g_audio_id, &kVolumeMainPropertyAddress, &handler, NULL);

        AudioObjectRemovePropertyListener(
            g_audio_id, &kVolumeLeftPropertyAddress, &handler, NULL);
    }

    AudioObjectAddPropertyListener(new_id, &kMuteMainPropertyAddress, &handler,
                                   NULL);

    AudioObjectAddPropertyListener(new_id, &kMuteLeftPropertyAddress, &handler,
                                   NULL);
...
void begin_receiving_volume_events() {
    ...
    g_audio_id = id;
    AudioObjectAddPropertyListener(id, &kMuteLeftPropertyAddress, &handler,
                                   NULL);

    AudioObjectAddPropertyListener(id, &kMuteMainPropertyAddress, &handler,
                                   NULL);

    AudioObjectAddPropertyListener(id, &kVolumeLeftPropertyAddress, &handler,
                                   NULL);

    AudioObjectAddPropertyListener(id, &kVolumeMainPropertyAddress, &handler,
                                   NULL);
