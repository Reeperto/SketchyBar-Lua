...
static void handle_domain_subscribe(FILE *rsp, struct token domain,
                                    char *message) {
    struct token name = get_token(&message);

    int item_index_for_name =
        bar_manager_get_item_index_for_name(&g_bar_manager, name.text);
...
static void handle_domain_trigger(FILE *rsp, struct token domain,
                                  char *message) {
    ...
    while (token.text && token.length > 0) {
        struct key_value_pair key_value_pair =
            get_key_value_pair(token.text, '=');

        if (key_value_pair.key && key_value_pair.value) {
            env_vars_set(&env_vars, string_copy(key_value_pair.key),
...
static void handle_domain_rename(FILE *rsp, struct token domain,
                                 char *message) {
    struct token old_name = get_token(&message);
    struct token new_name = get_token(&message);
    int item_index_for_old_name =
        bar_manager_get_item_index_for_name(&g_bar_manager, old_name.text);
    int item_index_for_new_name =
        bar_manager_get_item_index_for_name(&g_bar_manager, new_name.text);
...
static void handle_domain_clone(FILE *rsp, struct token domain, char *message) {
    ...
    int parent_index =
        bar_manager_get_item_index_for_name(&g_bar_manager, parent.text);
...
static void handle_domain_add(FILE *rsp, struct token domain, char *message) {
    ...
    if (token_equals(command, COMMAND_ADD_ITEM)) {
        ...
        if (bar_item->type == BAR_COMPONENT_GRAPH) {
            ...
            struct key_value_pair key_value_pair =
                get_key_value_pair(tmp_name, ',');
            if (!key_value_pair.key || !key_value_pair.value)
                alias_setup(&bar_item->alias, token_to_string(name), NULL);
            else
                alias_setup(&bar_item->alias, string_copy(key_value_pair.key),
                            string_copy(key_value_pair.value));
            ...
            while (member.text && member.length > 0) {
                ...
                if (member.length > 1 && member.text[0] == REGEX_DELIMITER &&
                    ...
                    int index = bar_manager_get_item_index_for_name(
                        &g_bar_manager, member.text);

                    if (index >= 0) {
                        bar_items =
                            realloc(bar_items, sizeof(struct bar_item *));
                        bar_items[0] = g_bar_manager.bar_items[index];
                        count = 1;
                    } else {
                        respond(rsp,
                                "[?] Add (Group) %s: Failed to add member "
                                "'%s', item not found\n",
                    ...
                    for (int i = 0; i < count; i++) {
                        if (first) {
                            if (bar_items[i]->position == POSITION_POPUP) {
                                popup_add_item(&bar_items[i]->parent->popup,
                                               bar_item);
...
static void handle_domain_default(FILE *rsp, struct token domain,
                                  char *message) {
    ...
    if (token_equals(command, PROPERTY_MARGIN)) {
        ...
        if (token_equals(token, ARGUMENT_WINDOW)) {
            ...
            needs_refresh = bar_manager_set_topmost(
                &g_bar_manager, TOPMOST_LEVEL_ALL,
        ...
        needs_refresh = bar_manager_set_sticky(
            &g_bar_manager,
            evaluate_boolean_state(token, g_bar_manager.sticky));
        ...
        if (position.length > 0)
            needs_refresh =
                bar_manager_set_position(&g_bar_manager, position.text[0]);
        ...
        needs_refresh = background_parse_sub_domain(&g_bar_manager.background,
                                                    rsp, command, message);
...
static char *reformat_batch_key_value_pair(struct token token) {
    ...
    if (!key_value_pair.key)
        return NULL;
    char *rbr_msg =
        malloc((strlen(key_value_pair.key) +
                (key_value_pair.value ? strlen(key_value_pair.value) : 0) + 3) *
               sizeof(char));
...
static void handle_domain_move(FILE *rsp, struct token domain, char *message) {
    ...
    int item_index =
        bar_manager_get_item_index_for_name(&g_bar_manager, name.text);
    int reference_item_index =
        bar_manager_get_item_index_for_name(&g_bar_manager, reference.text);
    ...
    bar_manager_move_item(&g_bar_manager, g_bar_manager.bar_items[item_index],
...
static void handle_domain_order(FILE *rsp, struct token domain, char *message) {
    ...
    memset(ordering, 0,
           sizeof(struct bar_item *) * g_bar_manager.bar_item_count);
    ...
    while (command.text && command.length > 0) {
        if (token_equals(command, DOMAIN_SET)) {
            ...
            if (!bar_items || count == 0) {
                ...
                while (token.text && token.length > 0) {
                    for (int i = 0; i < count; i++) {
                        struct token tmp = {string_copy(token.text),
                                            token.length};
                        char *rbr_msg = reformat_batch_key_value_pair(tmp);
                        free(tmp.text);
                        if (!rbr_msg) {
                            respond(rsp,
                                    "[!] Set (%s): Expected <key>=<value> "
                                    "pair, but got: '%s'\n",
            ...
            while (token.text && token.length > 0) {
                ...
                if (!rbr_msg) {
                    respond(rsp,
                            "[!] Set (default): Expected <key>=<value> pair, "
                            "but got: '%s'\n",
                            token.text);
                    break;
                }
            ...
            struct token token = get_token(&message);
            hotload_set_state(
                evaluate_boolean_state(token, hotload_get_state()));
