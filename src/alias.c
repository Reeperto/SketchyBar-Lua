...
void print_all_menu_items(FILE *rsp) {
    ...
    if (__builtin_available(macOS 11.0, *)) {
        if (!CGRequestScreenCaptureAccess()) {
            respond(rsp,
                    "[!] Query (default_menu_items): Screen Recording "
    ...
    for (int i = 0; i < window_count; ++i) {
        ...
        CFNumberRef layer_ref =
            CFDictionaryGetValue(dictionary, kCGWindowLayer);
        ...
        CFNumberGetValue(owner_pid_ref, CFNumberGetType(owner_pid_ref),
                         &owner_pid);
...
void alias_get_permission(struct alias *alias) {
    ...
    if (__builtin_available(macos 10.15, *)) {
        CGImageRef img = CGWindowListCreateImage(
            CGRectMake(0, 0, 1, 1), kCGWindowListOptionOnScreenOnly,
...
static void alias_find_window(struct alias *alias) {
    ...
    for (int i = 0; i < window_count; ++i) {
        ...
        CFNumberRef window_id_ref =
            CFDictionaryGetValue(dictionary, kCGWindowNumber);
        ...
        uint64_t wid;
        CFNumberGetValue(window_id_ref, CFNumberGetType(window_id_ref), &wid);
...
static bool alias_update_image(struct alias *alias, bool forced) {
    if (alias->window.id == 0)
        alias_find_window(alias);
    if (alias->window.id == 0)
        return false;
